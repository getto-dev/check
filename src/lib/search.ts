...
const score=(item:Pick<CatalogItem,'name'|'description'>,q:string)=>{if(!q.trim())return 1;const text=`${item.name} ${item.description}`.toLowerCase();return tokenizeQuery(q).reduce((s,t)=>s+(text.includes(t)?(item.name.toLowerCase().includes(t)?10:4):0),0)};

export const searchCatalog=(catalog:Record<string,Service[]>,query:string,categoryId?:string):CatalogItem[]=>
  Object.entries(catalog)
    .flatMap(([cat,items])=>categoryId&&cat!==categoryId?[]:items.map(i=>({id:i.id,categoryId:cat,name:i.n,description:i.d,unit:i.u,priceKopecks:i.p*100})))
    .map(i=>({...i,__score:score(i,query)}))
